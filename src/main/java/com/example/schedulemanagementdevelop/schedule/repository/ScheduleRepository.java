...
public interface ScheduleRepository extends JpaRepository<Schedule, Long> {
    @Query("SELECT s FROM Schedule s JOIN s.user WHERE s.user.isDeleted = false ORDER BY s.modifiedAt DESC")
    List<Schedule> findAllActiveUserSchedules();

    @Query("SELECT s FROM Schedule s JOIN s.user WHERE s.user.userName = :userName AND s.user.isDeleted = false ORDER BY s.modifiedAt DESC")
